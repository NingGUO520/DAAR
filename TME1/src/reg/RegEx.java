...
public class RegEx {
	...
	private static void etape2(RegExTree tree) {
		...
		int tab_epsilon[] = new int[nombreEtats];
		int cpt = 0;
		cpt = construireAutomate(tree,tab_trans ,tab_init ,tab_final ,tab_epsilon ,cpt);
	...
	private static int construireAutomate(RegExTree tree,int[][] tab_trans,int tab_init[],
			int tab_final[],int tab_epsilon[], int cpt) {
		int root = tree.root;
		if(root == RegEx.CONCAT) {
			
			
				cpt = construireAutomate(tree.subTrees.get(0),tab_trans,tab_init,tab_final,tab_epsilon,cpt);
				
				tab_epsilon[cpt-1] = cpt;
				
				cpt = construireAutomate(tree.subTrees.get(1),tab_trans,tab_init,tab_final,tab_epsilon,cpt);
