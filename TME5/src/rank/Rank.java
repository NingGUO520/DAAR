...
public class Rank {
  ...
  public static void main(String[] args){
    ...
    for (int i=0;i<6;i++) {
      for (int j=0;j<6;j++) System.out.print(tab[i][j]+" ");
      System.out.println();
    }
    System.out.println();
    System.out.println();

    double v[] = new double[] {1/(double)6,1/(double)6,1/(double)6,1/(double)6,1/(double)6,1/(double)6};
    double vNext[] = new double[] {1/(double)6,1/(double)6,1/(double)6,1/(double)6,1/(double)6,1/(double)6};

    for (int r=0;r<300;r++) {
      for (int i=0;i<6;i++) System.out.print(v[i]+" ");
      System.out.println();
      for (int i=0;i<6;i++) {
        vNext[i]=0;
        for (int j=0;j<6;j++) {
